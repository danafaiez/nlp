##This code uses a human_rating.txt dataset, computes the cosine and euclidean difference between each pair of target words, using both BERT and Word2Vec embeddings.
...
file = "ratings.txt"  #"2test_human_rate_texts.txt" "5test_human_rate_texts.txt"  
...
def index_list(Sent_Token,word_token):
  answer = []
  i=0
  k=0
  while k < (len(Sent_Token)):
    while i < (len(word_token)):
      if Sent_Token[k] == word_token[i]:
        answer.append(k)
        if i == len(word_token):
          return answer
        else:
...
with open(file, "r") as f:
  ...
  for t in range(len(paragraphs)-1):
    ...
    for w in split_with_punc[120-13:len(split_with_punc)-11]:
      sentence1_with_punc += w + ' ' ### This will be in input 1 for bert 
    #print("sentence1_with_punc:",sentence1_with_punc) 
    #print("split_with_punc:",split_with_punc) 
    ...
    if target_words[0] in vocabulary and target_words[1] in vocabulary:   
      ...
      for i in range(len(target_words)):
        ...
        if TargetWord in token: ### The  +1  is there becasue the 0th case in out belongs to the cls token. 
          ...
          indices = index_list(token,word_token)
          for k in indices:
            #print(k)
...
pd.DataFrame(Data_matrix).to_csv("/content/nlp/Human_rated_cos_norm_euc_Data_matrix_new.csv")
